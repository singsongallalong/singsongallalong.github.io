...
module.exports = class {
	...
	for (const item of memes) {
		...
		description,
		models: models
		    .filter((m) => (m.data.memes || []).includes(item.fileSlug))
		    .map(getModel),
	    });
	}
	...
	for (const item of themes) {
		...
		description,
		models: models
		    .filter((m) => (m.data.themes || []).includes(item.fileSlug))
		    .map(getModel),
	    });
	}
	debug("blah")
	return "blah" // JSON.stringify({ entries }, null, 2);
    }
};
